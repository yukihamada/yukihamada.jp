#!/usr/bin/env python3
"""Generate per-scene cloned-voice MP3s for the Transformer kamishibai (EP7).
Reads scripts/transformer_scenes.json -> public/audio/transformer-kam-{1..9}.mp3
Usage: python3 scripts/gen_transformer_audio.py [scene_numbers...]
"""
    ...
    for attempt in range(4):
        try:
            ...
            with urllib.request.urlopen(req, timeout=60) as r:
                audio = r.read()
            out = os.path.join(OUT, f"transformer-kam-{i}.mp3")
            with open(out, "wb") as f:
                f.write(audio)
            print(f"  transformer-kam-{i}.mp3  {len(audio)//1024}KB  OK")
            return True
